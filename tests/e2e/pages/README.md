...
| `shadow_closed.html`          | input inside a **closed** shadow root             | **Normal** (opaque to injection, backlog E2) |
...
| `iframe_same_origin.html`     | input inside a same-origin `iframe`               | **excluded** (backlog E1)                    |
| `iframe_srcdoc.html`          | input inside a `srcdoc` iframe                    | **excluded** (backlog E1)                    |
...
| `autofocus.html`              | page autofocuses on load, no user gesture         | Insert                                       |

The last row is the positive assertion that page-driven focus reaches Insert
(the harness drives no input at all — the page fires its own `.focus()`); the
`non_text_inputs.html` row is the inverse assertion: entering Insert there is
the bug. A suite that only checks "did we enter Insert" would pass while the
browser trapped the user in Insert every time they ticked a checkbox.
