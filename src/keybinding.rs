...
fn parse_unit(name: &str) -> Result<PageAction, KeyBindingError> {
    ...
    Ok(match name {
        ...
        "paste_url_after" => PasteUrl { after: true },
        "paste_url_before" => PasteUrl { after: false },
...
fn action_to_string(action: &PageAction) -> String {
    ...
    match action {
        ...
        PasteUrl { after: true } => "paste_url_after".into(),
        PasteUrl { after: false } => "paste_url_before".into(),
        TabReorder { from, to } => format!("tab_reorder(from = {from}, to = {to})"),
        MoveTabLeft => "move_tab_left".into(),
        MoveTabRight => "move_tab_right".into(),
        ...
        YankSelection => "yank_selection".into(),
        ZoomIn => "zoom_in".into(),
